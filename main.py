...
def _process_xml_file(file_path):
    ...
    if schema.validate(xml_tree) | True:
        return _process_tree(xml_tree)


def __get_value_from_elements(list_of_elements, look_for_this_name):
...
def _translate_tab_item(xml_element):
    # Hier bedarf es noch Vorgaben wie es hinterher aussehen soll. Entsprechen verbleiben Todos

    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
...
def _translate_group_item(xml_element):
    # Hier bedarf es noch Vorgaben wie es hinterher aussehen soll. Entsprechen verbleiben Todos
    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
...
def _translate_date(xml_element):
    ...
    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
    make_key = __generate_uuid(7) + "_key"
    my_date_description = ""
    ...
    if __get_value_from_elements(long_string_attributes, 'value') is not None:
        my_date_description = __get_value_from_elements(long_string_attributes, 'value')

    disabled_value = not __get_value_from_elements(boolean_attributes, 'editable').capitalize() == 'True'
    readonly_value = __get_value_from_elements(boolean_attributes, 'readOnly').capitalize() == 'True'
...
def _translate_textarea(xml_element):
    # make Id elements
    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
    make_key = __generate_uuid(7) + "_key"
    ...
    if __get_value_from_elements(long_string_attributes, 'description') is not None:
        description_attribute = __get_value_from_elements(long_string_attributes, 'description')

    # default value of the element
    if __get_value_from_elements(string_attributes, 'emptyText') is not None:
        default_attribute = __get_value_from_elements(string_attributes, 'emptyText')

    # min length of the element
    if __get_value_from_elements(number_attributes, 'minLength') is not None:
        min_length_attribute = __get_value_from_elements(number_attributes, 'minLength')

    # max length of the element
    if __get_value_from_elements(number_attributes, 'maxLength') is not None:
        min_length_attribute = __get_value_from_elements(number_attributes, 'maxLength')

    # pattern to check against of the element
    if __get_value_from_elements(string_attributes, 'regex') is not None:
        pattern_attribute = __get_value_from_elements(string_attributes, 'regex')

    # readonly bool of the element
    readonly_attribute = __get_value_from_elements(boolean_attributes, 'readOnly').capitalize == 'True'

    # required value of the element
    required_attribute = __get_value_from_elements(boolean_attributes, 'allowBlank').capitalize == 'True'
...
def _translate_timespinner(xml_element):
    datefield_label_element = xml_element.find('./label')
...
def _translate_radiobutton(xml_element):
    # make Id elements
    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
    make_key = __generate_uuid(7) + "_key"
    ...
    input_str = __get_value_from_elements(array_attributes, 'data')
    ...
    # default attribute
    default_attribute = literal_eval(__get_value_from_elements(custom_attributes, 'value'))[0]
...
def _translate_textfield(xml_element):
    # Todo: check if there are any other changes
    return _translate_textarea(xml_element)


def _translate_boolean_combo(xml_element):
    # make Id elements
    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
    make_key = __generate_uuid(7) + "_key"
    ...
    # disabled attribute
    if __get_value_from_elements(string_attributes, 'value') is not None:
        disabled_attribute = __get_value_from_elements(string_attributes, 'value').capitalize == "True"

    # readonly attribute
    readonly_attribute = __get_value_from_elements(boolean_attributes, 'readOnly').capitalize == 'True'

    # required attribute was not set in test data
    required_attribute_bool = __get_value_from_elements(boolean_attributes, 'allowBlank').capitalize == 'True'
...
def _translate_combo(xml_element):
    row_number = _generate_random_string(6)
    make_id = "Field_" + __generate_uuid(7)
    make_key = __generate_uuid(7) + "_key"
    ...
    # disabled was not set in test data
    disabled_attribute = __get_value_from_elements(boolean_attributes, 'editable').capitalize == 'True'
...
def _map_element(xml_element):
    # Find the 'name' child element and get its text content
    match xml_element.find('./xtype').text:
        case 'ddtabitem':
            return _translate_tab_item(xml_element)
        case 'ddgroup':
            return _translate_group_item(xml_element)
        case 'tpl-datefield':
            return _translate_date(xml_element)
        case 'tpl-textarea':
            return _translate_textarea(xml_element)
        case 'tpl-label':
            return translate_label(xml_element)
        case 'tpl-timespinner':
            return _translate_timespinner(xml_element)
        case 'tpl-radiobtn':
            return _translate_radiobutton(xml_element)
        case 'tpl-textfield':
            return _translate_textfield(xml_element)
        case 'tpl-booleancombo':
            return _translate_boolean_combo(xml_element)
        case 'tpl-combo':
            return _translate_combo(xml_element)
        case _:
            print("Could not map element")


def _process_panel(panel):
    combination = []
    # Check if xtype has the value "ddtabitem"
    xtype = panel.find('./xtype').text
    if xtype == 'ddtabitem':
        # Call mapElement function (to be implemented later)
        combination.append(_map_element(panel))
    else:
        print("panel has wrong format")

        # Iterate through FormGroup elements within the Panel
    for form_group in panel.findall('./FormGroup'):
        combination.append(_map_element(form_group))  # Call mapElement for each FormGroup

        # Iterate through FormField elements within the FormGroup
        for form_field in form_group.findall('./formField'):
            combination.append(_map_element(form_field))  # Call mapElement for each FormField
    return combination


def _write_json_to_file(data, file_path):
...
def _process_tree(xml_tree):
    panels = []
    for panel in xml_tree.findall('.//Panel'):
        panels.append(_process_panel(panel))
    return panels
...
def main():
    for xml_file in xml_files:
        ...
        data = _process_xml_file(xml_file), _put_meta(name_of_file)
        _write_json_to_file(data, f"./{name_of_file}.JSON")
